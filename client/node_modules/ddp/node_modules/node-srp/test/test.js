...
(function() {
  ...
  var request = C.startExchange();
  assert.equal(request.A, "");

  var challenge = S.issueChallenge(request);
  assert.equal(challenge.B, "");

  var response = C.respondToChallenge(challenge);
  assert.equal(response.M, "8705d31bb61497279adf44eef6c167dcb7e03aa7a42102c1ea7e73025fbd4cd9");

  var confirmation = S.verifyResponse(response);
  assert.equal(confirmation.HAMK, "07a0f200392fa9a084db7acc2021fbc174bfb36956b46835cc12506b68b27bba");

  assert.equal(C.verifyConfirmation(confirmation), true);
})();


(function() {
  console.log("srp - options");
  ...
  assert.equal(verifier.identity, 'a');
  assert.equal(verifier.salt, 'b');
  assert.equal(verifier.verifier, '3');

  var C = new srp.Client('c', clientOptions);
  var S = new srp.Server(verifier, serverOptions);

  var request = C.startExchange();
  assert.equal(request.A, '4');

  var challenge = S.issueChallenge(request);
  assert.equal(challenge.identity, 'a');
  assert.equal(challenge.salt, 'b');
  assert.equal(challenge.B, '7');

  var response = C.respondToChallenge(challenge);
  assert.equal(response.M, '471');

  var confirmation = S.verifyResponse(response);
  assert(confirmation);
  assert.equal(confirmation.HAMK, '44711');
})();
